...
input.forEach((line) => {
  line.tos.forEach((to) => {
    tos.push(`${line.from}->${to}`);
  });
});
// print tos and put into graphviz

const ffj = input.find((line) => line.from === "ffj");
const ffjR = ffj.tos.indexOf("lkm");
ffj.tos.splice(ffjR, 1);
const vgs = input.find((line) => line.from === "vgs");
const vgsR = vgs.tos.indexOf("xjb");
vgs.tos.splice(vgsR, 1);
const ljl = input.find((line) => line.from === "ljl");
const ljlR = ljl.tos.indexOf("xhg");
ljl.tos.splice(ljlR, 1);

/*
const toRemove = [
  input.findIndex((line) => line.from === "ffj"),
  input.findIndex((line) => line.from === "vgs"),
  input.findIndex((line) => line.from === "ljl"),
].sort((a, b) => b - a);
...
input.forEach((line) => {
  line.tos = line.tos.filter((to) => to !== "ffj" && to !== "vgs" && to !== "ljl");
});
*/

const input2 = input.map((line) => [line.from, ...line.tos]);
...
function visit(nodes) {
  nodes.forEach((node) => {
    ...
    visited.add(node);
    const nodesList = input2.filter((line) => line.includes(node));
...
console.log((visited.size) * (total.size - visited.size));
