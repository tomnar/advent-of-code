const rawInput = `Game 1: 1 red, 3 blue, 11 green; 1 blue, 5 red; 3 blue, 5 green, 13 red; 6 red, 1 blue, 4 green; 16 red, 12 green
...
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green`; // Should be 8
const testConfig = { red: 12, green: 13, blue: 14 };
  ...
  line
    ...
    .map((game) =>
      game.split(", ").map((color) => {
        ...
        return { [values[1]]: parseInt(values[0]) };
      })
    )
);

function isPossible(game, config) {
  for (const color of game) {
    const key = Object.keys(color)[0];
    if (config[key] < color[key]) return false;
  }
  return true;
}

const possibleGames = input
  .map((games) => games.map((game) => isPossible(game, testConfig)))
  .map((subArray) => !subArray.includes(false));

const count = possibleGames.reduce(
  (acc, val, idx) => (val ? acc + idx + 1 : acc),
  0
);
