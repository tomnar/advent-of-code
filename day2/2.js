...
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green`; // Should be 2286
...
const getLowVals = input.map((game) => {
  const lowVals = { red: 0, blue: 0, green: 0 };
  game.forEach((round) => {
    round.forEach((color) => {
      const key = Object.keys(color)[0];
      lowVals[key] = Math.max(lowVals[key], color[key]);
    });
  });
  return lowVals;
});

const calculatePower = getLowVals.map((game) => {
  return game.red * game.blue * game.green;
});

const calculateTotal = calculatePower.reduce(
  (acc, val) => val + acc,
  0
);
