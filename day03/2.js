const raw = `.....664...998........343...............851............................2............414.....................3....................948.164....
...
input.forEach((line, lineIdx) => {
  ...
  line.split("").forEach((char, j) => {
    if (char === "*") {
      if (!foundNr) {
        numberPos.push({start: j, end: line.length - 1});
      }
      foundNr = true;
    } else {
      if (foundNr) {
        numberPos[numberPos.length - 1].end = j - 1;
      }
      foundNr = false;
    }
  });

  numberPos.forEach((pos) => {
    const values = {
      top: [],
      bottom: [],
      left: [],
    ...
    for (let i = pos.start - 1; i < pos.end + 2; i++) {
      values.top.push(input[lineIdx - 1]?.charAt(i));
    }
    values.left.push(input[lineIdx].charAt(pos.start - 1));
    values.right.push(input[lineIdx].charAt(pos.end + 1));
    for (let i = pos.start - 1; i < pos.end + 2; i++) {
      values.bottom.push(input[lineIdx + 1]?.charAt(i));
    }

    let touchingNumbers = 0;
    ['top', 'bottom'].forEach((position) => {
      values[position].forEach((value, idx) => {
        if (!isNaN(value) && (idx === 0 || isNaN(values[position][idx - 1]))) {
          touchingNumbers++;
        }
      });
    });
    ...
    ['left', 'right'].forEach((position) => {
      if (!isNaN(values[position][0])) {
        touchingNumbers++;
      }
    });
    
    if(touchingNumbers === 2) {
      gearsWithTwoNumbers.push({lineIdx, ...pos, ...values})
    }
  });
});

const gearNumbers = [];
gearsWithTwoNumbers.forEach((gear) => {
  const validNumbers = [];
  if (isNaN(gear.top[1])) {
    const leftNr = [];
    let counter = 0;
    while (!isNaN(input[gear.lineIdx - 1][gear.start - 1 + counter])) {
      leftNr.push(Number(input[gear.lineIdx - 1][gear.start - 1 + counter]));
      counter--;
    }
    if (leftNr.length > 0) {
      validNumbers.push(leftNr.reverse().join(""));
    }

    const rightNr = [];
    counter = 0;
    while (!isNaN(input[gear.lineIdx - 1][gear.start + 1 + counter])) {
      rightNr.push(Number(input[gear.lineIdx - 1][gear.start + 1 + counter]));
      counter++;
    }
    if (rightNr.length > 0) {
      validNumbers.push(rightNr.join(""));
    }
  } else {
    const nr = [];
    let counter = 0;
    while (!isNaN(input[gear.lineIdx - 1][gear.start + counter])) {
      nr.push(Number(input[gear.lineIdx - 1][gear.start + counter]));
      counter--;
    }
    nr.reverse();
    counter = 1;
    ...
    validNumbers.push(nr.join(""));
  }
  if (isNaN(gear.bottom[1])) {
    const leftNr = [];
    let counter = 0;
    while (!isNaN(input[gear.lineIdx + 1][gear.start - 1 + counter])) {
      leftNr.push(Number(input[gear.lineIdx + 1][gear.start - 1 + counter]));
  ...
  if (!isNaN(gear.left[0])) {
    const leftNr = [];
    let counter = 0;
    while (!isNaN(input[gear.lineIdx][gear.start - 1 + counter])) {
      leftNr.push(Number(input[gear.lineIdx][gear.start - 1 + counter]));
      counter--;
    }
    if (leftNr.length > 0) {
      validNumbers.push(leftNr.reverse().join(""));
  ...
  if (!isNaN(gear.right[0])) {
    const rightNr = [];
    counter = 0;
    while (!isNaN(input[gear.lineIdx][gear.start + 1 + counter])) {
      rightNr.push(Number(input[gear.lineIdx][gear.start + 1 + counter]));
      counter++;
    }
    if (rightNr.length > 0) {
      validNumbers.push(rightNr.join(""));
  ...
  gearNumbers.push(validNumbers);
});

const ratios = gearNumbers.map((nr) => nr[0] * nr[1]);

const calculateTotal = ratios.reduce(
  (acc, val) => val + acc,
  0
);
