...
let res = 0;
input.forEach(i => {
    const n1 = i.replaceAll('one', 'one1one');
    const n2 = n1.replaceAll('two', 'two2two');
    const n3 = n2.replaceAll('three', 'three3three');
    const n4 = n3.replaceAll('four', 'four4four');
    const n5 = n4.replaceAll('five', 'five5five');
    const n6 = n5.replaceAll('six', 'six6six');
    const n7 = n6.replaceAll('seven', 'seven7seven');
    const n8 = n7.replaceAll('eight', 'eight8eight');
    const n9 = n8.replaceAll('nine', 'nine9nine');
    const numbers = n9.replace(/\D/g, '');
    
    if (numbers.length === 0) {
        return;
    } else {
        const calc = `${numbers[0]}${numbers[numbers.length - 1]}`;
        res = res + Number(calc);
    }
