...
input.forEach((line, lineIdx) => {
  ...
  numberPos.forEach((pos) => {
    ...
    for (let i = pos.start - 1; i < pos.end + 2; i++) {
      values.push(input[lineIdx - 1]?.charAt(i));
      values.push(input[lineIdx + 1]?.charAt(i));
    }
    values.push(input[lineIdx].charAt(pos.start - 1));
    values.push(input[lineIdx].charAt(pos.end + 1));
    const hasOtherValue = values.some(value => value !== undefined && value !== '' && value !== '.');
    if (hasOtherValue) {
      const number = [];
      for (let i = pos.start; i <= pos.end; i++) {
        number.push(input[lineIdx].charAt(i));
      }
      validNumbers.push(Number(number.join('')));
    } else {
      const number = [];
      for (let i = pos.start; i <= pos.end; i++) {
        number.push(input[lineIdx].charAt(i));
...
const calculateTotal = validNumbers.reduce(
  (acc, val) => val + acc,
  0
);
