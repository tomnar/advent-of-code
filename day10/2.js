...
const raw = `FF--7-|7F--7F--|7F77-.FJF7F-7-F|7.77.FF7-F7---7-FJ7.|J--FJJ-FF-.JJ7LF7.J7F-7-F77L7.7-7.7FF77FF7..F--L7JF--.FF.7F|JFL7.7FL77.-7|7.F7-7FJ.FJ.-
...
const position = getStartPosition(raw); // { x: number, y: number }
const startSymbol = getStartSymbol(position);
...
function getStartPosition(input) {
    const startIndex = input.replaceAll('\n', '').split('').findIndex(char => char === 'S');
    const rowLength = input.split('\n')[0].length;
    return { x: startIndex % rowLength, y: Math.floor(startIndex / rowLength) };
}

function go(direction, position) {
    switch (direction) {
        case 'N':
            return {...position, y: position.y - 1, symbol: getSymbol(position.x, position.y - 1)};
        case 'E':
            return {...position, x: position.x + 1, symbol: getSymbol(position.x + 1, position.y)};
        case 'S':
            return {...position, y: position.y + 1, symbol: getSymbol(position.x, position.y + 1)};
        case 'W':
            return {...position, x: position.x - 1, symbol: getSymbol(position.x - 1, position.y)};
...
function getStartSymbol(pos) {
    const configuration = [];
    const mapper = { NS: '|', EW: '-', NE: 'L', NW: 'J', SW: '7', ES: 'F'}
    const validPaths = { north: ['|', '7', 'F'], east: ['-', 'J', '7'], south: ['|', 'L', 'J'], west: ['-', 'F', 'L']}
    if (validPaths.north.includes(getSymbol(pos.x, position.y - 1))) configuration.push('N');
    if (validPaths.east.includes(getSymbol(pos.x + 1, pos.y))) configuration.push('E');
    if (validPaths.south.includes(getSymbol(pos.x, pos.y + 1))) configuration.push('S');
    if (validPaths.west.includes(getSymbol(pos.x - 1, pos.y))) configuration.push('W');

    let validSymbols = Object.entries(mapper).filter(([key, value]) => {
        return [...key].every(char => configuration.includes(char));
    }).map(([key, value]) => value);

    if(validSymbols.length != 1) {
        console.log('Invalid start symbol', pos, validSymbols)
    }
    return validSymbols[0];
}

path.push({...position, symbol: startSymbol});
let lastExit = null;
while (true) {
    const position = path[path.length - 1];
    const mapper = { NS: '|', EW: '-', NE: 'L', NW: 'J', SW: '7', ES: 'F'}
    const entryMapper = { N: 'S', E: 'W', S: 'N', W: 'E' }

    if (path.length === 1) {
        const exits = Object.entries(mapper).find(([key, value]) => value === position.symbol)?.[0].split('');
        lastExit = entryMapper[exits[0]];
    }

    const exits = Object.entries(mapper).find(([key, value]) => value === position.symbol)?.[0].split('');
    const entry = entryMapper[lastExit];
    const direction = exits.filter(exit => exit !== entry)[0];

    lastExit = direction;
    const nextPosition = go(direction, position);
...
const positions = path.map(pos => ({x: pos.x, y: pos.y}));
...
function findEnclosedPoints(polygon) {
    let points = [];
    ...
    for (let x = minX; x <= maxX; x++) {
        for (let y = minY; y <= maxY; y++) {
            let point = {x: x, y: y};
            if (isPointInPolygon(point, polygon) && !isPointOnPolygon(point, polygon)) {
                points.push(point);
            }
        }
    }
...
const points = findEnclosedPoints(positions);
const mazeCopy = JSON.parse(JSON.stringify(maze)).map(line => line.map(_ => '.'));
positions.forEach((pos, idx) => mazeCopy[pos.y][pos.x] = getMazeSymbol(path[idx].symbol));
const image = mazeCopy.map(l => l.join('')).join('\n');


points.forEach(point => {
   mazeCopy[point.y][point.x] = 'O';
});
const image2 = mazeCopy.map(l => l.join('')).join('\n');

// console.log(image)
// console.log(image2) // Very pretty :)
